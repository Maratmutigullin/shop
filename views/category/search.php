...
<div class="container mt-5">
    <h2 class="text-center">Вы искали:<?= Html::encode($search)  ?></h2>
    <div class="row m-0 d-flex position-relative">
                ...
                <?php $i = 0; foreach($products as $product):  ?>
                    <div class="card col-lg-3">
                        ...
                        <div class="card-body position-relative">
                            <h5 class="card-title card-text text-center"><a class="hit" href="<?= \yii\helpers\Url::to(['product/view', 'id'=>$product->id])  ?>"><?= $product->name ?></a>></h5>
                            <span  class="price d-flex justify-content-center font-weight-bolder "><?= $product->price ?> р</span>
                        </div>
                        <div class=" bike-one-search">
                            <a href="<?= \yii\helpers\Url::to(['web/cart/add', 'id'=>$product->id]) ?>" data-id="<?= $product->id ?>" class="text-white d-flex justify-content-center cart-js card_link "><button type="button" class="btn text-white btn_cart  text-body">Купить</button></a>
                        </div>
            ...
            <?php else: ?>
                <h3 style="height: 500px">Ничего не найдено</h3>
            <?php endif;?>
